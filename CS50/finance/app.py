...
uri = "postgres://yciaivzkkyfuyr:048aac195e99a595f8d18d88ff7755182f3af188108e4e4d47b77c827f79573a@ec2-107-23-76-12.compute-1.amazonaws.com:5432/dan0rjkv9ckaju"
...
def buy():
    ...
    if request.method == "POST":
        ...
        currentUser = session["user_id"]
        userCashTable = db.execute(
            "SELECT cash FROM users WHERE id = ?", currentUser)
        ...
        userTransactionsTableID = (
            "user_" + (str(currentUser)) + "_transactions")
        userPortfolioTableID = ("user_" + (str(currentUser)) + "_portfolio")
        ...
        try:
            ...
            db.execute(
                "CREATE TABLE ? (stockSymbol TEXT PRIMARY KEY NOT NULL, stockName TEXT UNIQUE, sharesOfStock NUMERIC NOT NULL)", userPortfolioTableID)
        ...
        db.execute("INSERT INTO ? (Date, transactionType, stockSymbol, stockPrice, numShares, transactionAmount) VALUES (?, 'buy', ?, ?, ?, ?)",
                   userTransactionsTableID, datetime.datetime.now(), symbol, price, shares, requestedBuyAmount)
        db.execute("UPDATE users SET cash=? WHERE id=?",
                   (userCashAmount - requestedBuyAmount), currentUser)

        # update users portfolio table based on transaction
        # first search if user already has existing shares of the stock
        currentPortfolio = db.execute(
            "SELECT stockSymbol FROM ? WHERE stockSymbol=?", userPortfolioTableID, symbol)

        if not currentPortfolio:
            # create a new entry in users portfolio
            db.execute("INSERT INTO ? (stockSymbol, stockName, sharesOfStock) VALUES (?, ?, ?)",
                       userPortfolioTableID, symbol, name, shares)
        else:
            # update the current portfolio
            db.execute("UPDATE ? SET sharesOfStock = sharesOfStock + ? WHERE stockSymbol = ?",
                       userPortfolioTableID, shares, symbol)
...
def register():
    ...
    if request.method == "POST":
        ...
        # Check that the username is available and does not already exist
            ...
            # note returns list of dictionaries
        ...
        attemptedPassword = generate_password_hash(
            request.form.get("password"))
        db.execute("INSERT INTO users (username, hash) VALUES (?, ?)",
                   attemptedUsername, attemptedPassword)
...
def sell():
    ...
    userStockSymbols = db.execute(
        "SELECT stockSymbol FROM ?", userPortfolioTableID)
    ...
    if request.method == "POST":
        ...
        sharesInput = request.form.get("shares")
        numSharesAvailable = db.execute(
            "SELECT sharesOfStock FROM ? WHERE stockSymbol=?", userPortfolioTableID, selectedSymbol)
        ...
        db.execute("INSERT INTO ? (Date, transactionType, stockSymbol, stockPrice, numShares, transactionAmount) VALUES (?, 'sell', ?, ?, ?, ?)",
                   userTransactionsTableID, datetime.datetime.now(), symbol, price, shares, requestedSellAmount)

        # Update users cash amount and portfolio tables
        db.execute("UPDATE users SET cash = cash + ? WHERE id = ?",
                   requestedSellAmount, currentUser)
        db.execute("UPDATE ? SET sharesOfStock = sharesOfStock - ? WHERE stockSymbol = ?",
                   userPortfolioTableID, shares, symbol)

        # Remove the stock completely from the portfolio if 0 shares are left
        db.execute("DELETE FROM ? WHERE sharesOfStock = 0",
                   userPortfolioTableID)
